...
void automatic_pilotage(int n_pieces) {
    for (int i = 0; i < n_pieces; i++) {
        printf("\n--- Traitement piece %d / %d ---\n", i + 1, n_pieces);

        //Amener la piece
        convoyeur_on();
        Sleep(2000); // Temps simule pour que la piece arrive devant le robot
        convoyeur_off();

        //Aller au point de saisie (Pick)
        printf("Deplacement vers Pick...\n");
        robot_move(426500, -191600, 39300, 195000000, 18000000, -10000000);

        // Petit delai pour laisser le temps au simulateur de mettre a jour l'etat 'moving'
        Sleep(100);

        // Attente avec securite (Timeout)
        int timeout = 0;
        while (robot_is_moving() && timeout < 100) {
            Sleep(50);
            timeout++;
        }

        vacuum_on();
        Sleep(500); // Simulation du temps de prise (aspiration)

        // Aller au point de depot (Place)
        printf("Deplacement vers Place...\n");
        robot_move(155000, -497000, -50000, 180000000, 0, 0);
        ...
        Sleep(300); // Temps de relachement
    }
}
