...
unsigned char compute_checksum(Message* msg) {
    ...
    // 2. Sérialisation des champs dans le buffer
    buffer[index++] = msg->type;
    buffer[index++] = msg->number;
    buffer[index++] = msg->length;

    // 3. Copie uniquement des données réellement utilisées
    for (int i = 0; i < msg->length; i++) {
        buffer[index++] = msg->data[i];
    }

    // 4. Calcul du checksum sur le contenu du buffer
...
{
    const int MAX_ATTEMPTS_SHORT = 200;   // ~20 s
    const int MAX_ATTEMPTS_LONG  = 1000;  // ~100 s
    ...
    for (int i = 0; i < n_pieces; i++)
    {
        ...
        while (state != PS_DONE)
        {
            switch (state)
            {
                ...
                //--------------------------------------------------
                // WAIT ROBOT ARRIVAL AT PICK
                //--------------------------------------------------
                case PS_WAIT_MOVE_TO_PICK:

                    if (robot_is_moving() == 0)
                    {
                        printf("Robot at pick position.\n");
                    ...
                    else
                    {
                        ...
                        {
                            printf("Timeout robot move to pick.\n");
                            state = PS_DONE;
                        }
                    }
                ...
                //--------------------------------------------------
                // WAIT ROBOT ARRIVAL AT PLACE
                //--------------------------------------------------
                case PS_WAIT_MOVE_TO_PLACE:

                    if (robot_is_moving() == 0)
                    {
                        printf("Robot at place position.\n");
                    ...
                    else
                    {
                        ...
                        if (++attempts > MAX_ATTEMPTS_LONG)
                        {
                            printf("Timeout robot move to place.\n");
                            state = PS_DONE;
                        }
                    }
                ...
                case PS_WAIT_VACUUM_OFF:
                    ...
                    if (get_has_piece() == 0)
                    {
                        printf("Piece %d deposited.\n", i + 1);

                        //--------------------------------------------------
                        // IMPORTANT:
                        // wait until pallet leaves the sensor
                        //--------------------------------------------------
                        ...
                        while (get_pallet_sensor() == 1)
                        {
                            Sleep(100);

                            if (++attempts > MAX_ATTEMPTS_SHORT)
                            {
                                printf("Timeout pallet evacuation.\n");
                                break;
                            }
                        }
    ...
    printf("Automatic cycle finished.\n");
}
